...
pub enum UnescapeError {
  ...
  LoneEscape,
  #[error("not enough number for octal escape")]
  NotEnoughNumOctal,
  #[error("not enough number for hex escape")]
  NotEnoughNumHex,
  #[error("not enough number for unicode escape")]
  NotEnoughNumUnicode,
...
pub fn unescape(s: impl Into<String>) -> Result<String, (UnescapeError, usize)> {
  let s: String = s.into();
  ...
  while let Some(ch) = chars.next() {
    ...
    if ch == '\\' {
      let next_ch = chars.next().ok_or((UnescapeError::LoneEscape, pos))?;
      pos += 1;
      ...
      let esc_ch = match next_ch {
        ...
        '0'..='7' if matches!(chars.peek(), Some('0'..='7')) => {
          let first = next_ch.to_digit(8).ok_or((UnescapeError::InvalidOctal, pos))?;

          let second = chars.next().ok_or((UnescapeError::NotEnoughNumOctal, pos))?;
          pos += 1;
          let second = second.to_digit(8).ok_or((UnescapeError::InvalidOctal, pos))?;

          let third = chars.next().ok_or((UnescapeError::NotEnoughNumOctal, pos))?;
          pos += 1;
          let third = third.to_digit(8).ok_or((UnescapeError::InvalidOctal, pos))?;
        ...
        'x' => {
          let first = chars.next().ok_or((UnescapeError::NotEnoughNumHex, pos))?;
          pos += 1;
          let first = first.to_digit(16).ok_or((UnescapeError::InvalidHexInHexEscape, pos))?;

          let second = chars.next().ok_or((UnescapeError::NotEnoughNumHex, pos))?;
          pos += 1;
          let second = second.to_digit(16).ok_or((UnescapeError::InvalidHexInHexEscape, pos))?;
        ...
        'u' => {
          ...
          for _ in 0..4 {
            let digit = chars
              .next()
              .ok_or((UnescapeError::NotEnoughNumUnicode, pos))?
              .to_digit(16)
              .ok_or((UnescapeError::InvalidHexInUnicodeEscape, pos))?;
            pos += 1;
        ...
        _ => return Err((UnescapeError::InvalidEscape, pos)),
      };
...
fn test_unescape() {
  macro_rules! assert_unescape {
    ($input:expr => UnescapeError::$expected:tt) => {
      assert_eq!(unescape($input).map_err(|(err, _)| err), Err(UnescapeError::$expected))
    };
  ...
  assert_unescape!("\\x1" => UnescapeError::NotEnoughNumHex);
